...
export function CheckoutPage() {
  ...
  async function handlePostcodeBlur() {
    ...
    try {
      ...
      setForm((previous : any) => {
        if(previous.postcode.trim() !== postcode || !result) {
          return previous
        }
  ...
  async function handleSubmit(event: ChangeEvent) {
    ...
    try {
      const {order : order} = await apiRequest<{order: PlacedOrder}>('/orders', {
        method: 'POST',
        token: token,
        body: {
          quantity: quantity,
  ...
  return (
    <main className="mx-auto max-w-2xl px-6 py-12">
      ...
      <div className="mt-6 rounded-2xl bg-white p-5 shadow-sm">
          ...
          <dl className="mt-3 space-y-1 text-sm text-plum/70">
              <div className="flex justify-between">
                  <dt>Sweet Surpise Gift Bag x {quantity}</dt>
                  <dd>£{total}</dd>
              </div>
              {personalMessage && (
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-5">
          <FormField id="name" label="Name" value={form.name} error={errors.name} onChange={(value) => updateField('name', value)}></FormField>
          <FormField id="email" label="Email" type="email" value={form.email} error={errors.email} onChange={(value) => updateField('email', value)}></FormField>
          <FormField id="address" label="Delivery Address" value={form.address} error={errors.address} onChange={(value) => updateField('address', value)}></FormField>
          <div className="grid gap-5 sm:grid-cols-2">
            <FormField id="city" label="City / Town" value={form.city} error={errors.city} onChange={(value) => updateField('city', value)}></FormField>
            <FormField id="postcode" label="Postcode" value={form.postcode} error={errors.postcode} onChange={(value) => updateField('postcode', value)} onBlur={handlePostcodeBlur}></FormField>
            <PostcodeStatusMessage status={postcodeStatus}></PostcodeStatusMessage>
          </div>

          {submitError && (
            <p role="alert" className="text-sm text-berry-dark">{submitError}</p>
          )}

          <button type="submit" disabled={submitting} className="w-full rounded-full bg-berry px-6 py-3 font-semibold text-white shadow-sm transition hover:bg-berry-dark disabled:opacity-60 sm:w-auto">
