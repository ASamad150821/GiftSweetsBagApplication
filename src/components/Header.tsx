          ...
          <div className="flex items-center gap-3">
            <nav aria-label="Primary" className="flex gap-1 rounded-full bg-white/70 p-1">
              {[...navItems, ...(user ? authedNavItems : [])].map((item) => (
                <NavLink key={item.to} to={item.to} end={item.end} className={({ isActive : isActive }) => 
            ...
            {user ? (
              <button type="button" onClick={handleLogout} className="rounded-full px-4 py-1.5 text-sm font-medium text-plum/70 hover:bg-white">
                Log Out
              </button>
            ) : (
              <Link to="/login" className="rounded-full px-4 py-1.5 text-sm font-medium text-plum/70 hover:bg-white">Log In</Link>
            )}
